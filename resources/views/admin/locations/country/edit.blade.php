            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <h4 class="mb-0">{{__("message.countries_edit")}}</h4>
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item active">{{__("message.countries_edit")}}</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <form action="{{route('countries.update',$location->id)}}" class="needs-validation" method="post"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" name="country_code" id="country" required="">
                                                    ...
                                                    @foreach($countries as $country)
                                                        <option value="{{$country['code']}}" {{$country['name'] == $location->getTranslation('title','en') ? 'selected' : ''}}>{{$country['name']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            @error('country_code')
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" name="currency_code" id="currency_code" required="">
                                                    ...
                                                    @foreach($currencies as $currency)
                                                        <option value="{{$currency['cc']}}" {{$currency['cc'] == $location->currency_code ? 'selected' : ''}}>{{$currency['name']}}({{$currency['symbol']}})</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            @error('currency_code')
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-md-4 col-lg-4">
                                            <div class="form-check">
                                                <input class="form-check-input" name="status" type="checkbox" {{$location->status ? 'checked' : ''}}  id="defaultCheck1">
